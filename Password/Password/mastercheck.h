...
{
	if (alphabetOptions(argv) != 0)//проверка сосуществования -С и -а
		return 1;
	if (alphabetCheck(argv) != 0) // проверка аргументов -С и -а
		return 1;
	if (lengthOptions(argv) != 0)// проверка сосуществования -m1 -m2 и n
		return 1;
	if (otherOptions(argv) != 0)// проверка на повторы -n и -с
		return 1;
	if (argsAreNumbers(argv) != 0)// проверка числовых аргументов после  -m1 -m2 -n -c 
		return 1;
	if (minMaxComparison(argv) != 0)//проверка соответствия значений -m1 -m2 (m1<m2)
		return 1;
